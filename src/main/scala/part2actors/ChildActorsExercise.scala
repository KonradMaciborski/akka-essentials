...
object ChildActorsExercise extends App {
  ...
  class WordCounterMaster extends Actor {
    ...
    def withChildren(childrenRefs: Seq[ActorRef], currentChildIndex: Int, currentTaskId: Int, requestMap: Map[Int, ActorRef]): Receive = {
      case text: String =>
        println(s"[master] I have received: $text - I will send it to childe $currentChildIndex")
        val originalSender = sender()
        val task = WordCountTask(currentTaskId, text)
        val childRef = childrenRefs(currentChildIndex)
        childRef ! task
        val nextIndex = (currentChildIndex + 1) % childrenRefs.length
  ...
  class TestActor extends Actor {
    ...
    override def receive: Receive = {
      case "go" =>
        val master = context.actorOf(Props[WordCounterWorker], "master")
        master ! Initialize(3)
